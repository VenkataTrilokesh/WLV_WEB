...
function runKWL(graph, k, sharedHashMap = null) {
  ...
  // ---------------------------------------------------------------
  // FIX: Map tuple colors -> per-node color using the DIAGONAL tuple
  // (i, i, i, ..., i) for each node i. This is the canonical choice
  // in k-WL literature and avoids the frequency-collapse bug where
  // high-k tuple counts cause one color to dominate and all nodes
  // render as the same color (visible as k=5 showing all-purple).
  //
  // Old approach: pick the MOST FREQUENT tuple color among all tuples
  //   containing node i — for k=5 with n^5 tuples this collapses
  //   because one structural color appears far more often than others.
  //
  // New approach: use the color of the diagonal tuple (i,i,...,i).
  //   The diagonal tuple's color directly encodes the node's structural
  //   equivalence class under k-WL because it captures all pairwise
  //   adjacency/equality patterns in its signature. Falls back to the
  //   minimum color among tuples whose first element is i, if needed.
  // ---------------------------------------------------------------
  function nodeColorsFromTupleColors(tColors) {
    const result = {};
    for (let i = 0; i < n; i++) {
      // Primary: use the diagonal tuple (i, i, ..., i)
      const diagTuple = new Array(k).fill(i);
      const diagKey   = tupleKey(diagTuple);
      if (tColors[diagKey] !== undefined) {
        result[i] = tColors[diagKey];
      } else {
        // Fallback: minimum color among tuples starting with node i
        // (min is stable and doesn't collapse like frequency-max)
        let minColor = Infinity;
        for (const t of tuples) {
          if (t[0] === i) {
            const c = tColors[tupleKey(t)];
            if (c !== undefined && c < minColor) minColor = c;
          }
        }
        result[i] = minColor === Infinity ? 0 : minColor;
      }
    }
    return result;
  }
  ...
  // Max iterations = n*k + 2 (safe upper bound for convergence), capped at 50
  ...
  for (let iter = 0; iter < MAX_ITER; iter++) {
    ...
    // Break BEFORE updating state or pushing a snapshot when nothing changed.
    if (!changed) break;
